...
const { getAllApprovedUsers } = useApprovedUsersStore();
const { approved_users, is_loading, paginationConfig, selected_user } = storeToRefs(useApprovedUsersStore());
...
watch(
 () => page.value,
 async () => await getAllApprovedUsers({ page: page.value, limit: limit.value, keyword: keyword.value }),
);

watch(
 () => selected_user.value,
 async () => {
  if (!selected_user.value) await getAllApprovedUsers({ page: page.value, limit: limit.value, keyword: keyword.value });
 },
);

const searchKeyword = useDebounce(async () => {
 await getAllApprovedUsers({ page: page.value, limit: limit.value, keyword: keyword.value });
}, 500);
     ...
     <div class="relative flex flex-col min-w-0 break-words shadow bg-clip-border rounded-2xl bg-light/30">
      <div class="px-9 pt-5 flex justify-between items-stretch flex-wrap min-h-[70px] pb-0 bg-transparent">
       <h3 class="flex flex-col items-start justify-center m-2 ml-0 font-medium text-xl/tight text-dark">
        <span class="mr-3 font-semibold text-dark">Verified User Accounts</span>
        <span class="mt-1 font-normal text-secondary-dark text-sm">Manage all approved accounts registered here.</span>
       </h3>
       ...
       <div :class="is_loading ? 'overflow-hidden' : 'overflow-x-auto'">
        <table class="w-full my-0 align-middle text-dark border-neutral-200">
         ...
         <tbody>
          ...
          <tr v-if="!is_loading && approved_users.length <= 0">
           <td
            colspan="6"
          ...
          <tr
           v-for="item in approved_users"
           v-if="!is_loading && approved_users.length > 0"
           :key="item.id"
