...
const { getAllPendingUsers } = usePendingUsersStore();
const { pending_users, is_loading, paginationConfig, selected_user } = storeToRefs(usePendingUsersStore());
...
watch(
 () => page.value,
 async () => await getAllPendingUsers({ page: page.value, limit: limit.value, keyword: keyword.value }),
);

watch(
 () => selected_user.value,
 async () => {
  if (!selected_user.value) await getAllPendingUsers({ page: page.value, limit: limit.value, keyword: keyword.value });
 },
);

const searchKeyword = useDebounce(async () => {
 await getAllPendingUsers({ page: page.value, limit: limit.value, keyword: keyword.value });
}, 500);
     ...
     <div class="relative flex flex-col min-w-0 break-words shadow bg-clip-border rounded-2xl bg-light/30">
      <div class="px-9 pt-5 flex justify-between items-stretch flex-wrap min-h-[70px] pb-0 bg-transparent">
       <h3 class="flex flex-col items-start justify-center m-2 ml-0 font-medium text-xl/tight text-dark">
        <span class="mr-3 font-semibold text-dark">Pending User Accounts</span>
        <span class="mt-1 font-normal text-secondary-dark text-sm">Manage all pending accounts registered here.</span>
       </h3>
        ...
        <table class="w-full my-0 align-middle text-dark border-neutral-200">
         <thead class="align-bottom">
          <tr class="font-semibold text-[0.95rem] text-secondary-dark">
           ...
           <th class="pb-3 pr-12 text-end min-w-[100px]">Registration Date</th>
          </tr>
         </thead>
         <tbody>
          <tr v-if="is_loading">
          ...
          <tr v-if="!is_loading && pending_users.length <= 0">
           <td
            colspan="6"
          ...
          <tr
           v-for="item in pending_users"
           v-if="!is_loading && pending_users.length > 0"
           :key="item.id"
          ...
          >
           ...
           <td class="pr-12 text-end">
            <span class="text-light-inverse text-md/normal">{{ moment(item.created_at).format("LLL") }}</span>
           </td>
          </tr>
         </tbody>
        </table>
